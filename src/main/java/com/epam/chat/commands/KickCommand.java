...
public class KickCommand implements Command {
    ...
    @Override
    public void execute(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException, NullPointerException {
        HttpSession httpSession = req.getSession();
        User admin = (User) httpSession.getAttribute(ParameterNames.USER_SESSION);
        String adminName = admin.getNick();
        String userName = req.getParameter(ParameterNames.KICK_USER);
        UserServiceImpl.getInstance().kick(adminName, userName);
